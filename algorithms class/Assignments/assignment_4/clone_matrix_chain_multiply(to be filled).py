...
def lookup_chain(m, p, i, j):
	...
	if i == j:
		...
		# 매트릭스 체인 A[i]...A[k]와 A[k+1]...A[j]의 부분 문제에 대한 최소 곱셈 비용을 조회합니다.
		# 여기서 p[i - 1] * p[k] * p[j]는 A[i]...A[k]와 A[k+1]...A[j]를 곱하는데 필요한 스칼라 곱셈 횟수입니다.
		for k in range(i, j):
...
#Q4 code
	...
	print("Bottom-Up Dynamic Programming Time:", bottom_up_time)
	# print_optimal_parens(s, 1, n)
	# print_m(m, n)
	# print_s(s, n)
	print("Minimum Scalar Multiplications (Bottom-Up):", int(m[1, n]))
	...
	print("Minimum Scalar Multiplications (Recursive):", min_multiplications_recursive)

	# Testing memoized dynamic programming approach
	start_time = time.time()  # Start timer
	min_multiplications_memoized = memoized_matrix_chain(p, n)
	memoized_time = time.time() - start_time  # Calculate elapsed time
	print("Memoized Dynamic Programming Time:", memoized_time)
	print("Minimum Scalar Multiplications (Memoized):", min_multiplications_memoized)

	# Compare and explain the results
	print("\nComparison of Running Times:")
	print("Bottom-Up DP:", bottom_up_time)
	print("Recursive:", recursive_time)
	print("Memoized DP:", memoized_time)